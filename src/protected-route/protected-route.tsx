...
export const ProtectedRoute = ({ onlyUnAuth = false, children }: ProtectedRouteProps) => {
  ...
  if (!onlyUnAuth && !user) {
    ...
    // редирект на страницу регистрации (вместо логина)
    return <Navigate replace to="/registration" state={{ from: location }} /> // в поле from объекта location.state записываем информацию о URL
  }
